...
export default function PaginatedContentViewer({ topic, subtopic }) {
  ...
  // Engagement detection from webcam frames
  useEffect(() => {
    if (!showCamera || !user) {
      console.log('📷 Engagement detection disabled:', { showCamera, hasUser: !!user });
      return;
    }

    console.log('📷 Starting engagement detection...');
    ...
    const captureAndDetect = async () => {
      ...
      try {
        ...
        console.log('📷 Sending frame to engagement API...');
        
        // Send to backend for engagement detection
        const response = await api.detectEngagement({
          user_id: user.user_id,
          image_base64: imageBase64
        });

        if (response.data) {
          console.log(`📷 Engagement: ${(response.data.engagement_score * 100).toFixed(0)}% (face: ${response.data.face_detected})`);
          
          // Update engagement level
          setEngagementLevel(prev => {
            const alpha = 0.4;
            const newScore = alpha * response.data.engagement_score + (1 - alpha) * prev;
            console.log(`📷 Focus updated: ${(prev * 100).toFixed(0)}% -> ${(newScore * 100).toFixed(0)}%`);
            return newScore;
          });
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    // Note: Engagement is now detected via webcam in separate useEffect
    // This WebSocket is kept for future real-time features but won't override engagement
    cognitiveService.connect((data) => {
      // WebSocket connected - engagement detection handled by camera capture
      console.log('📡 Cognitive WebSocket active');
    });
